package cloud66

import (
	"fmt"
	"net/http"
	"testing"

	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource"
	mockHttp "github.com/moemoe89/go-helpers"
)

func TestAccCloud66SslCertificate_LetsEncrypt(t *testing.T) {
	t.Parallel()

	rnd := generateRandomResourceName()
	stackID := generateRandomUid()
	uid := generateRandomUid()

	srv := mockHttp.HttpMock("/stacks/"+stackID+"/ssl_certificates.json", http.StatusOK, testAccCloud66SslCertificateLetsEncrypt(uid))
	defer srv.Close()

	resourceName := "cloudflare_custom_ssl." + rnd
	resource.Test(t, resource.TestCase{
		Providers: testAccProviders,
		Steps: []resource.TestStep{
			{
				Config: testAccCloud66SslCertificate_LetsEncrypt(srv.URL, stackID, rnd),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "stack_id", stackID),
					resource.TestCheckResourceAttr(resourceName, "name", "my-serv-new"),
					resource.TestCheckResourceAttr(resourceName, "ca_name", "Let's Encrypt"),
					resource.TestCheckResourceAttr(resourceName, "type", "lets_encrypt"),
					resource.TestCheckResourceAttr(resourceName, "ssl_termination", "true"),
					resource.TestCheckResourceAttr(resourceName, "server_group_id", "null"),
					resource.TestCheckResourceAttr(resourceName, "server_names.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "intermediate_certificate", "null"),
					resource.TestCheckResourceAttr(resourceName, "has_intermediate_cert", "true"),
				),
			},
		},
	})
}

func testAccCloud66SslCertificate_LetsEncrypt(url string, stactID string, rnd string) string {
	return fmt.Sprintf(`
provider "cloud66" {
	url = "%[1]s"	
	access_token = "%[2]s"
}

resource "cloud66_ssl_certificate" "%[4]s" {
	stack_id = "%[3]s"
	type = "lets_encrypt"
	server_names = "example.com"
}
`, url, testAccCloud66AccessToken, stactID, rnd)
}

func TestAccCloud66SslCertificate_Manual(t *testing.T) {
	t.Parallel()

	rnd := generateRandomResourceName()
	stackID := generateRandomUid()
	uid := generateRandomUid()

	srv := mockHttp.HttpMock("/stacks/"+stackID+"/ssl_certificates.json", http.StatusOK, testAccCloud66SslCertificateManual(uid))
	defer srv.Close()

	resourceName := "cloudflare_custom_ssl." + rnd
	resource.Test(t, resource.TestCase{
		Providers: testAccProviders,
		Steps: []resource.TestStep{
			{
				Config: testAccCloud66SslCertificate_Manual(srv.URL, stackID, rnd),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "stack_id", stackID),
					resource.TestCheckResourceAttr(resourceName, "name", "my-serv-new"),
					resource.TestCheckResourceAttr(resourceName, "ca_name", "Let's Encrypt"),
					resource.TestCheckResourceAttr(resourceName, "type", "lets_encrypt"),
					resource.TestCheckResourceAttr(resourceName, "ssl_termination", "true"),
					resource.TestCheckResourceAttr(resourceName, "server_group_id", "null"),
					resource.TestCheckResourceAttr(resourceName, "server_names.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "intermediate_certificate", "null"),
					resource.TestCheckResourceAttr(resourceName, "has_intermediate_cert", "false"),
				),
			},
		},
	})
}

func testAccCloud66SslCertificate_Manual(url string, stactID string, rnd string) string {
	return fmt.Sprintf(`
provider "cloud66" {
	url = "%[1]s"
	access_token = "%[2]s"
}
	
resource "cloud66_ssl_certificate" "%[4]s" {
	stack_id = "%[3]s"
	type = "lets_encrypt"
	server_names = "example.com"
    certificate = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
    key = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
	`, url, testAccCloud66AccessToken, stactID, rnd)
}
